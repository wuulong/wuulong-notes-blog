...
OUTPUT_KML = os.path.join(SCRIPT_DIR, "../maps/2026_zhuoshui_river.kml")
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>{MAP_NAME}</name>
    <description>從上游到出海口的完整流域探索</description>
"""
    for f in features:
        name = f['name']
        lat, lon = f['coords']
...
def create_google_maps_link(features):
    ...
    # Google Maps URL has a limit, so we might not be able to put all 50 points.
    # We will put up to 10 significant ones or just print the list.
    # For now, let's try to generate it, but just be aware.
    # Actually, let's just create a link map centered on the first point or a search link?
    # No, the original logic was a route. 
    # Let's limit to the first 20 for the link to avoid URL length issues or browser rejects,
    # or just provide multiple links if needed.
