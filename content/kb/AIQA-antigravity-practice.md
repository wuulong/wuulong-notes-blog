...
# Q101.1-hook 的 class 清單
- M::
	- in notebooklm study#1: antigravity code pack。 
- Q::hook 的 class 清單，以及每個 class 的簡介
- A::
以下是 Google Antigravity SDK 中提供的 Hook 類別清單以及各個 class 的簡介，可依照其功能分類為以下幾種：

### **基礎與抽象 Hook 類別 (Base Hook Types)**

這些是定義 Hook 行為模式的泛型基底類別：

- **`InspectHook`**：唯讀、非阻塞（non-blocking）的 Hook，主要用於觀測與紀錄（observability）。
- **`DecideHook`**：唯讀、阻塞（blocking）的 Hook，主要用於策略與權限的決策（policy decisions），會回傳是否允許執行的結果。
- **`TransformHook`**：會修改資料的阻塞型 Hook，主要用於資料的轉換與加工。

### **核心生命週期 Hook (Lifecycle Hooks)**

這些 Hook 允許開發者在 Agent 運作的不同階段介入：

**Session（會話階段）**

- **`OnSessionStartHook`**：在整個會話（Session）開始時觸發。
- **`OnSessionEndHook`**：在整個會話（Session）結束時觸發。

**Turn（對話輪次階段）**

- **`PreTurnHook`**：在一個對話輪次開始前觸發，資料參數會接收使用者的提示（prompt），包含文字或多模態資料。
- **`PostTurnHook`**：在一個對話輪次結束後觸發，資料參數會接收模型在該輪次生成的最終回應文字。

**Tool（工具呼叫階段）**

- **`PreToolCallDecideHook`**：在工具呼叫（tool call）執行之前觸發，用來決定是否允許繼續執行該工具,。
- **`PostToolCallHook`**：在工具呼叫完成後觸發，會接收包含工具執行細節的 `ToolResult` 物件。
- **`OnToolErrorHook`**：當工具執行失敗時觸發，允許開發者進行錯誤復原，或修改模型最終看到的錯誤訊息呈現方式。

**其他互動與狀態事件**

- **`OnInteractionHook`**：當代理（Agent）需要與使用者互動時觸發（例如代理提出問題需要使用者回答）。
- **`OnCompactionHook`**：當發生上下文壓縮（context compaction）事件時觸發（即當上下文視窗超過設定的 token 閾值時），提供一個觀測點以便進行日誌記錄、指標分析或發送 UI 通知。

### **互動式與除錯用 Hook (Interactive CLI Hooks)**

這些 Hook 內建於 SDK 的 `utils.interactive` 中，主要用於終端機的本地開發與除錯：

- **`ToolConfirmationHook`**：繼承自 `PreToolCallDecideHook`，在執行工具呼叫前，會透過標準輸入（stdin）提示使用者確認是否允許執行,。
- **`AskQuestionHook`**：繼承自 `OnInteractionHook`，用於在終端機介面提示使用者回答 Agent 所提出的問題,。

### **OpenTelemetry (OTel) 追蹤 Hook**

這是一組專門用於觀測性的 Hook，負責將 SDK 的生命週期事件轉換為 OpenTelemetry 的追蹤資料（Traces 與 Spans）：

- **`OTelSessionStartHook` / `OTelSessionEndHook`**：負責建立與關閉根會話（Session）的 Span,。
- **`OTelPreTurnHook` / `OTelPostTurnHook`**：在 Session 下建立與關閉單一輪次（Turn）的 Span。
- **`OTelPreStepHook` / `OTelPostStepHook`**：負責內部步驟（Step）的 Span 建立與關閉。
- **`OTelPreToolCallHook` / `OTelPostToolCallHook`**：負責工具呼叫（Tool call）的 Span 建立與關閉。
- **`OTelOnToolErrorHook`**：負責在工具出錯時於 Span 中記錄例外狀況（Exception）並結束該工具的 Span。

### **內部遙測 Hook (Internal Telemetry Hooks)**

（僅供 SDK 內部遙測使用，一般開發者較少直接使用）

- **`_PreStepHook`**：當一個步驟（Step）首次出現在串流中時觸發。
- **`_PostStepHook`**：當一個步驟完成時觸發。
# Q101.2-agent 有哪幾種 class
- M::
- Q::agent 有哪幾種 class
- A::
在 Google Antigravity SDK 中，與 Agent 直接相關的 class 主要可以分為核心互動、配置設定以及子代理（Subagent）三種類別：

**1. 核心執行類別**

- **`Agent`**：這是 SDK 提供的 Layer 1 高階 API 類別，用於簡化與代理互動的流程。它是開發者主要實例化並用來執行任務、管理生命週期或進行對話（例如使用 `chat()` 方法）的主要入口點。

**2. 配置設定類別 (Configuration)**

- **`AgentConfig`**：這是一個抽象基底類別（Abstract base class），定義了所有 Agent 設定的通用介面與核心欄位（如模型、工具、策略與 Hook 等）。
- **`LocalAgentConfig`**：繼承自 `AgentConfig` 的具體實作類別，專門用於配置基於本地端（local harness）執行的後端代理，這也是 `Agent` 類別最常用的預設配置方式。

**3. 子代理相關類別 (Subagents)**

- **`SubagentConfig`**：用於設定靜態子代理（static subagent）的組態類別。開發者可以透過它定義子代理的獨立屬性，包含名稱、描述、專屬的系統指令（system instructions）、可用能力（capabilities）以及自訂工具（tools）。
- **`CustomAgent`**：這是底層 Protobuf 的配置類別，系統在內部會將開發者設定的 `SubagentConfig` 轉換為 `CustomAgent` 規格，以供底層引擎解析與建構子代理。

# Q101.3-response 內有哪些物件
- M::
- Q::response 內有哪些物件，如 response.thoughts
- A::
在 Google Antigravity SDK 中，呼叫 `Agent.chat()` 後所回傳的 response 物件為 **`ChatResponse`**。這是一個支援惰性緩衝（lazy buffering）的非同步語義區塊串流。

這個 `ChatResponse` 物件內包含了多個**屬性（Properties）**與**非同步方法（Async Methods）**，可以用來取得不同類型的串流資料或最終結果。具體包含以下物件與方法：

### **串流屬性 (Streaming Properties)**

這些屬性會回傳獨立的非同步迭代器（AsyncIterator），讓你可以即時取得串流中的不同內容：

- **`response.thoughts`**：回傳非同步的字串迭代器，提供**模型內部的推理與思考過程**（thinking token deltas）。
- **`response.chunks`**：回傳一個包含 `StreamChunk`、`ToolCall` 或 `ToolResult` 的非同步迭代器，提供**豐富且未過濾的語義區塊串流**，適用於進階的開發需求。
- **`response.tool_calls`**：回傳強型別 `ToolCall` 物件的非同步迭代器，讓你可以在模型派發工具時**即時取得工具呼叫的細節**。
- **直接迭代 ( `async for delta in response` )**：由於實作了 `__aiter__`，你可以直接對 response 進行非同步迭代，這會直接**串流回傳純文字對話內容**（text token deltas）。

### **靜態資料與元數據 (Metadata & Data)**

- **`response.usage_metadata`**：回傳 `UsageMetadata` 物件（或 `None`），包含該輪次對話中所有模型呼叫的**累積 Token 使用量統計**（如 prompt、candidates、thoughts 與總 token 數）。

### **非同步解析方法 (Async Resolution Methods)**

如果你不需要處理即時串流，可以使用這些方法直接等待並取得最終的完整結果：

- **`response.text()`**：排空整個串流，並回傳**最終聚合完成的完整對話字串**。
- **`response.structured_output()`**：排空串流，並回傳從對話完成步驟（FINISH step）中提取的**已解析結構化輸出（Structured Output）**，若無則回傳 `None`。
- **`response.resolve()`**：完整排空底層串流，並將所有的區塊（包含文字、思考、工具呼叫與結果）以**一個扁平的列表（List）回傳**。

### **控制方法 (Control Methods)**

- **`response.cancel()`**：**取消當前正在執行中的對話輪次**，並停止後續的生成作業。

# Q101.4-LocalAgentConfig 有哪些重要的 parametes
- M::
- Q::LocalAgentConfig 有哪些重要的 parametes
- A::
在 Google Antigravity SDK 中，`LocalAgentConfig` 繼承自基礎的 `AgentConfig`，是用於配置本地端執行 Agent 的主要類別。這個類別整合了代理的行為、權限、模型選擇與擴充能力。

以下是 `LocalAgentConfig` 中幾個最重要且常用的參數，依功能分類如下：

### **1. 模型與連線配置 (Model & Connection)**

這組參數用來決定 Agent 背後使用的語言模型及其驗證方式：

- **`model`**：一個便捷的參數，用來指定預設的文字對話模型名稱（例如字串 `"gemini-3.5-flash"`）。
- **`models`**：進階的模型清單設定，允許你同時配置多個 `ModelTarget`。例如，可以同時指派專門負責純文字對話的模型與負責圖片生成的模型。
- **`api_key`**：設定用於呼叫 Gemini API 的驗證金鑰。
- **`vertex`**, **`project`**, **`location`**：若你使用 GCP Vertex AI 作為模型後端，可以透過這三個參數啟用並指定你的 GCP 專案與地區設定。

### **2. 核心行為與能力 (Behavior & Capabilities)**

這組參數決定了 Agent 解決問題的基礎邏輯與可用工具的範圍：

- **`system_instructions`**：系統指令，用於賦予 Agent 特定身份、核心指引與操作限制。
- **`capabilities`**：接收一個 `CapabilitiesConfig` 物件，用來明確設定哪些「內建工具」應被啟用或隱藏（透過 `enabled_tools` 或 `disabled_tools`），以及是否允許 Agent 啟動子代理（Subagents）。
- **`response_schema`**：若需要 Agent 以特定的結構化格式回覆，可在此傳入 JSON 格式字串、字典（dict），或 Pydantic 的 `BaseModel` 來強制約束輸出格式。

### **3. 安全性與存取策略 (Security & Policies)**

在本地端執行時，保護系統安全非常重要，這些參數負責管理工具呼叫的權限：

- **`policies`**：指定工具調用的安全策略清單。預設情況下，它會使用 `confirm_run_command` 策略（允許所有工具，但呼叫 `run_command` 執行終端機指令前會拒絕或要求使用者確認）。若你需要 Agent 完全自動運行，可覆寫為 `policies=[policy.allow_all()]`。
- **`workspaces`**：一個路徑清單（預設為當下執行目錄 `os.getcwd()`）。設定後，系統會自動套用 `workspace_only` 策略，將 Agent 的所有檔案操作（如讀取、編輯、建立檔案）嚴格限制在這些指定的目錄內，防止 Agent 存取到系統其他敏感檔案。

### **4. 擴充功能與互動 (Extensions & Hooks)**

這些參數允許開發者為 Agent 增添新的能力或介入其生命週期：

- **`tools`**：開發者自訂的 Python 函數清單，Agent 可以將這些函數當作外部工具來呼叫。
- **`hooks`**：註冊自訂的 Hook 清單，這也是我們在先前對話中提到的，可用於在對話輪次（Turn）、工具呼叫（Tool Call）等不同階段介入並修改 Agent 行為。
- **`triggers`**：外部事件觸發器清單，可以讓 Agent 被動地對時間間隔（Cron）或檔案變動等外部事件做出反應。
- **`mcp_servers`**：Model Context Protocol (MCP) 伺服器的配置清單，允許 Agent 連接外部的 MCP Server 以獲取更多工具與能力。
- **`subagents`**：靜態子代理的配置清單，可用於建立能被主 Agent 喚醒的特定任務助手。

### **5. 狀態與儲存 (State & Storage)**

管理 Agent 的上下文、記憶體與資料存取位置：

- **`conversation_id`**：可指定一個會話識別碼，以便追蹤或延續特定的對話脈絡。
- **`save_dir`**：指定 Agent 儲存運行狀態、歷史記錄的目錄。若未指定，系統將會自動建立一個暫存目錄。
- **`app_data_dir`**：Agent 的應用程式專屬資料目錄，必須提供絕對路徑，常被用來存放長期的知識庫或大腦資料。

# Q101.5-policies
- M::
- Q::說明還能寫成什麼樣子： policies=[policy.allow_all()]
- A::
在 Google Antigravity SDK 中，`policies=[policy.allow_all()]` 代表**允許 Agent 自動執行所有工具（包含危險的終端機指令），不需要任何使用者確認**。

如果您不想要完全開放，`policies` 參數接收一個宣告式的策略列表，您可以針對不同工具設定**允許 (APPROVE)**、**拒絕 (DENY)** 或**詢問使用者 (ASK_USER)**。系統會根據「具體設定優先於萬用字元 (*)」的規則來攔截工具呼叫。

以下是 `policies` 其他常見的寫法與應用情境：

### 1. 預設安全策略 (Safe Defaults)

如果不特別傳入 `policies`，系統預設會使用 `policy.confirm_run_command()`。這會**允許大部分工具，但直接拒絕最危險的 `run_command`（執行終端機指令）**。 如果您希望 Agent 可以執行指令，但必須先經過您的同意，可以加入 `handler`：

```
policies = policy.confirm_run_command(handler=my_approval_fn)
```

_這會讓其他工具自動執行，但在呼叫 `run_command` 時觸發 `my_approval_fn` 詢問使用者_。

### 2. 預設全部拒絕，只開放特定工具 (Deny By Default)

您可以先使用 `policy.deny_all()` 封鎖所有工具，再利用 `policy.allow()` 逐一開放需要的工具，確保系統安全。

```
policies = [
    policy.deny_all(),              # 預設封鎖所有工具
    policy.allow("view_file"),      # 破例允許查看檔案
    policy.allow("list_directory")  # 破例允許列出目錄
]
```

### 3. 加入條件判斷動態拒絕 (Conditional Deny)

您可以透過 `when` 參數傳入一個 Lambda 或函式，根據 Agent 傳入的參數決定是否放行。例如，**允許執行指令，但攔截具破壞性的 `rm` 刪除指令**：
...
_當條件成立時會觸發拒絕，否則預設允許所有操作_。

### 4. 要求使用者手動授權 (Ask User)

如果您針對某些敏感工具（如發送 Email、存取資料庫等自訂工具）需要人工介入確認，可以使用 `ask_user`：

```
policies = [
    policy.ask_user("send_email", handler=my_approval_fn),
    policy.allow_all()
]
```

_Agent 呼叫 `send_email` 時會被暫停，並執行 `my_approval_fn` 等待回傳 True 才會繼續_。

### 5. 限制檔案操作的目錄範圍 (Workspace Only)

如果您希望 Agent 只能在特定的資料夾內讀寫檔案，可以使用 `workspace_only`，這會產生一組自動拒絕越權路徑的策略：

```
policies = policy.workspace_only(["/tmp/my_workspace"])
```

_所有針對 `/tmp/my_workspace` 以外的檔案操作（如 `edit_file`, `create_file` 等）都會被直接拒絕_。

### 💡 綜合應用範例

您可以將這些規則自由組合成一個列表，打造最適合您的安全邊界：

```
policies = [
    policy.deny("*"),                                     # 1. 預設阻擋所有工具
    policy.allow("read_file"),                            # 2. 允許讀取檔案
    policy.deny("run_command",                            # 3. 絕對禁止執行 rm 指令
        when=lambda args: "rm" in args.get("CommandLine", "")),
    policy.ask_user("run_command", handler=my_approval_fn) # 4. 其他指令皆須詢問使用者
]
```
