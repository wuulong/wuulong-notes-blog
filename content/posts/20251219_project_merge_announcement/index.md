...
graph TD
    %% === 樣式定義 ===
    classDef TimeLine fill:none,stroke:none,color:#666,font-size:14px;
    classDef River fill:#E1F5FE,stroke:#0277BD,stroke-width:2px;
    %% 藍色系 (河川)
    
    classDef Tech fill:#E8F5E9,stroke:#2E7D32,stroke-width:2px;
    %% 綠色系 (技術)
    
    classDef Merge fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,stroke-dasharray: 5 5;
    %% 紫色區 (合併)

    %% === 時期一：河川探索起源 ===
    subgraph Period1 [Phase 1: 起源 - 2026 台灣河川探索]
        direction TB
        R_Vision(數位河流學):::River
        R_Trip(大甲溪車宿/高美濕地):::River
        R_SimpleGIS(初步 Shapefile 實驗):::River
        
        R_Vision --> R_Trip
        R_Trip -.-> |需求浮現| R_SimpleGIS
    end

    %% === 時期二：技術擴張 (哈爸筆記誕生) ===
    subgraph Period2 [Phase 2: 擴張 - 哈爸筆記與技術堆疊]
        direction TB
        T_Mind(GenAI 思維模型):::Tech
        T_Infra(GCP/Zeabur 雲端架構):::Tech
        T_Agent(Agentic AI 實作):::Tech
        T_Comm(哈爸實驗室 Discord):::Tech
    ...
    %% === 時期三：匯流與賦能 (現在) ===
    subgraph Period3 [Phase 3: 匯流 - 技術賦能探索]
        direction TB
        M_Tool(Fabric + Gemini CLI):::Merge
        M_Data(River Buffer / SQL Schema):::Merge
        M_Eco(專案生態系整併):::Merge
    end
    ...
    %% 1. 需求驅動技術
    R_SimpleGIS --> |遭遇技術瓶頸| T_Infra

    %% 2. 技術反哺應用 (Merger)
    T_Agent ==> |自動化賦能| M_Tool
    M_Tool ==> |解決舊難題| M_Data
